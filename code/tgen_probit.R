...
# library(BayesLogit)
tgen_probit <- function (X, Z, y, family = c("gaussian","binomial"), sigma, sa,
  ...
  if (ns == 1) {
    ...
    out      <- probit_outerloop(X,Z,y,family,SZy,SZX,c(sigma),c(sa),c(logodds),
    ...
    if (initialize.params) {
      ...
      for (i in 1:ns) {
        ...
        out <- probit_outerloop(X,Z,y,family,SZy,SZX,sigma[i],sa[i],logodds[,i],
  ...
  if (family == "gaussian") {
    ...
    fit$model.pve <- tgen_probitpve(X,fit,nr)
    ...
    #fit$fitted.values <- probit.linear.predictors(X,Z,family,mu.cov,alpha,mu)
...
probit_outerloop <- function (X, Z, y, family, SZy, SZX, sigma, sa, logodds,
  ...
  if (family == "gaussian") {
    ...
    out <- tgen_probitnorm(X,y,sigma,sa,log(10)*logodds,alpha,mu,update.order,
...
probit.linear.predictors <- function (X, Z, family, mu.cov, alpha, mu) {
  ns <- ncol(alpha)
